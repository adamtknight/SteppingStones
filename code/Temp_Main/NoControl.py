...
text = font.render("Press TAB when aligned", True, (0, 0, 0))
...
# Draw the text message asking the user to press enter to start
font = pygame.font.Font(None, 30)
text = font.render("Press ENTER once you have started recording and want stepping stones to be displayed", True, (0, 0, 0))
text_rect = text.get_rect()
text_rect.center = (screen_width // 2, screen_height // 2)
screen.blit(text, text_rect)
...
global circle_x, circle_y, frame_number, color, stone_counter, side
stone_counter = 0
side = "right"
stone_counter+=1
...
async def wait_for_event(event_type):
    event = await connection.await_event(event=event_type)
    if event:
...
async def draw_stones():
    ...
    global connection, markers, frame_number, color, stone_counter, side
